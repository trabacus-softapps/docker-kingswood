...
#                wf_service.trg_validate(uid, 'account.invoice', i, 'invoice_open', cr)
    ...
    def do_merge_old_inv(self, cr, uid, ids, context=None):
        ...
        for case in self.browse(cr, uid, ids):
            ...
            cr.execute("select id from account_invoice where state='draft' and partner_id= '" + str(case.partner_id.id) + "'and partner_id in (select id from res_partner where handling_charges = True) and type='in_invoice' and EXTRACT(month from date_invoice) = '" + str(month_to) + "' and EXTRACT(year from date_invoice) ='" + str(year_to) + "'")
            other_sup=[x[0] for x in cr.fetchall()]  
            ...
            group_ids=self.search(cr,uid,[('partner_id','=',case.partner_id.id),('company_id','=',case.company_id.id),('state','=','draft'),('id','in',other_sup)],order='partner_id',)
            group_in_ids=self.search(cr,uid,[('origin','=','IN'),('partner_id','=',case.partner_id.id),('company_id','=',case.company_id.id),('state','=','draft'),('id','in',other_sup)],order='origin',)
...
#                 self.invoice_validate(cr, uid, group_in_ids, context)
    ...
    def do_merge_facilitator(self, cr, uid, automatic=False, use_new_cursor=False, context=None):
        ...
        if int(month_to)<8 and int(year_to)==2014:
            if supplier_id:
                cr.execute("select id from account_invoice where state='draft' and partner_id= '" + str(supplier_id) + "'and partner_id in (select id from res_partner where handling_charges = True) and type='in_invoice' and EXTRACT(month from date_invoice) < '" + str(8) + "' and EXTRACT(year from date_invoice) ='" + str(year_to) + "'")
            else:
                cr.execute("select id from account_invoice where state='draft' and partner_id in (select id from res_partner where handling_charges = True) and type='in_invoice' and EXTRACT(month from date_invoice) < '" + str(8) + "' and EXTRACT(year from date_invoice) ='" + str(year_to) + "'")
            other_sup=[x[0] for x in cr.fetchall()] 
        else:     
            if supplier_id:
            ...
            for case in self.browse(cr, uid, ids):
                ...
                if user:
                    if case.user_id.id not in user:
                        raise osv.except_osv(_('Warning!'), _("Cannot Merge Invoices with Different Salespersons"))
                else:
                ...
                if case.partner_id.handling_charges:
                    ...
                    self.write(cr,uid,res.keys(),{'date':today,'origin':'Handling Charges'},context=context)
...
#                         product2.append(case.product_id.id)
        ...
        #                 inv_ids.append(case.id)
            ...
            if del_ord_out_ids:
                self.write(cr,uid,res.keys(),{'date':today,'freight':case.freight,'branch_state':case.branch_state.id,'delivery_orders_ids': [(6,0,del_ord_out_ids)]},context=context) 
                                                           
            if del_ord_in_ids:
                self.write(cr,uid,res.keys(),{'date':today,'freight':case.freight,'branch_state':case.branch_state.id,'supp_delivery_orders_ids': [(6,0,del_ord_in_ids)]},context=context)  
            ...
            if shipment_ids:
                if not origin:
                    origin='IN'
                self.write(cr,uid,res.keys(),{'date':today,'incoming_shipment_ids': [(6,0,shipment_ids)],'origin':origin,'branch_state':case.branch_state.id,},context=context)
            if res:
                inv_ids =res.keys()   
...
#         return super(account_invoice,self).unlink(cr, uid, ids,context)
    ...
    def create_facilitator_inv(self, cr, uid, ids, context=None):
        ...
        today = context.get('shedular_date',False)
        ...
        if not today:        
            today = time.strftime('%Y-%m-%d')
        shedular_date = datetime.strptime(today, '%Y-%m-%d') - relativedelta(days=int(2))
        shedular_date = shedular_date.strftime('%Y-%m-%d')
...
#         stock_ids_out = stock_obj.search(cr,uid,[('id','in',stock_ids_out),('paying_agent_id','not in',dummy_ids)])
        ...
        cr.execute("""select distinct sp.id from stock_picking sp 
                        ...
                        and sp.date::date <= '"""+str(shedular_date)+"""'::date and sup_invoice = False                        
                        and sp.state in ('done','freight_paid') and lower(sp.name) like '%ka%'""")
 
        stock_ids_in=cr.fetchall()
        if stock_ids_in:
            stock_ids_in=zip(*stock_ids_in)[0]           
#         stock_ids_in = [x[0] for x in cr.fetchall()]
         
#         stock_ids_in = [75449, 75448, 75653, 75652, 75651, 75650, 75649, 75278, 75277, 75276, 75870, 75648, 75647, 75646, 75645, 75643, 75641, 75640, 75638, 75637]        
        
              
#         stock_ids = stock_obj.search(cr,uid,[('sup_invoice','=',False),('type','=','out'),('state','in',('done','freight_paid')),('date','<=',str(shedular_date))],order='paying_agent_id,delivery_date_function')
        print len(stock_ids_in),'-stock_ids_in',stock_ids_in
        print len(stock_ids_out),'-stock_ids_out',stock_ids_out
        print "Date-",shedular_date
...
#                     if order[0] in stock_ids_out:
#                         stock_ids_out.remove(order[0])
            if stock_ids_in:
...
#                 for in_shipment_id in in_shipment_ids:
#                     if in_shipment_id[0] in stock_ids_in:
#                         stock_ids_in.remove(in_shipment_id[0])
                                             
#             for stock in stock_obj.browse(cr,uid,stock_ids):
#                 print "DC-",str(stock.name),"state",stock.state_id.name
#                 supp_key = stock.paying_agent_id.id,stock.delivery_date_function,stock.product_id.id
#                 print supp_key
#                 if supp_key not in st:
#                     st_ids.append(stock.id)
#                     st[supp_key]=st_ids
#                 else:
#                     st[supp_key].append(stock.id)    
#                 st_ids = []
#                   
#                 
#             print "DC-st",st
...
#                 st_id = st[key]
            ...
            print len(stock_ids_out),'-stock_ids_out',stock_ids_out  
#             stock_ids = [75603, 75602, 75599, 75598, 75597, 75596, 75594, 75593, 75582, 75581, 75575, 75573, 75572, 75571, 75569, 75566, 75564, 75563, 75561, 75559, 75550]
    ...
    #             stock_ids_in = []
                ...
                for sh_month in out_shedular_month_ids:
                    ...
                    if context.get('invoices',[]) not in invoice_created_in:
...
#                 self.write(cr,uid,i_id,{'back_date':True})
                for case in self.browse(cr,uid,i_id):
                    ...
                    if case.partner_id.id not in partner:
                        partner.append(case.partner_id)
                    for lines in case.invoice_line:
                        key = case.partner_id.id,lines.price_unit,month_to,case.type,case.company_id.id,case.origin,lines.product_id.id
                        if key not in invoices:
                            inv_ids.append(case.id)
                            invoices[key] = inv_ids
                        else:
                            invoices[key].append(case.id)
                        inv_ids = [] 
                        
                                
            print 'invoice to merge', invoices 
            print 'no. invoice to merge', len(invoices)   
            for merge_inv in invoices:
                ...
                if invoices_ids and len(invoices_ids)==1:
                    ...
                    cr.execute("""update account_invoice set back_date=True where id = %s """,(invoices_ids[0],))   
                    ...
                    more_inv.append(invoices_ids)
#                     invoices_ids.pop(merge_inv)
                    
            for merge_more in more_inv:
                inv_date = shedular_date
#                 invoices_ids = invoices[merge_inv]
                if merge_more and len(merge_more)>1: 
                    inv_done_m = self.browse(cr,uid,merge_more[0])
                    inv_done_to=self.get_month(cr,uid,[],inv_done.date_invoice,{'month':1})
                    if last_date_to != inv_done_to:
                        inv_date = last_date                    
                    self.do_merge(cr, uid, merge_more, context)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         
    ...
    #             context.update({'billing_ids':[{'partner_id':partner_id.id}]})
                billing_list = billing_obj.search(cr,uid,[('partner_id','=',partner_id.id)],order='end_date desc',limit=1)
...
#         if state_id1:
#             context.update({'state':'karnataka'})
#         if state_id2:
#             context.update({'state':'tamil'})            
...
# to validate the remaining invoices
    def validate_facilitator_inv(self, cr, uid, ids, context=None):
        print "context-",context
        if not context:
            context={}
        partner = []
        ...
        if not today:        
            today = time.strftime('%Y-%m-%d')
        shedular_date = datetime.strptime(today, '%Y-%m-%d') - relativedelta(days=int(4))
        shedular_date = shedular_date.strftime('%Y-%m-%d')
        cr.execute("select id from res_partner where lower(name) like 'dummy%'")
        dummy_ids=[y[0] for y in cr.fetchall()]
        print "Date-",shedular_date
        ...
        invoice_id = ids
        print 'invoice_id',invoice_id
        for case in self.browse(cr,uid,invoice_id):
            self.write(cr,uid,[case.id],{'date_invoice':shedular_date,'back_date':True})
            cr.execute("delete from billing_cycle where end_date = '2015-02-23'::date and partner_id ="+str(case.partner_id.id))
            print case.back_date
            if case.partner_id.id not in partner:
                partner.append(case.partner_id)
                if case.date_invoice:
                    month_to=self.get_month(cr,uid,[],case.date_invoice,{'month':1})
            for lines in case.invoice_line:
                key = case.partner_id.id,lines.price_unit,month_to,case.type,case.company_id.id,case.origin
                if key not in invoices:
                    inv_ids.append(case.id)
                    invoices[key] = inv_ids
                else:
        ...
        for merge_inv in invoices:
            ...
            if invoices_ids and len(invoices_ids)==1:
                print "validate single",invoices_ids
                self.write(cr,uid,[invoices_ids[0]],{'date_invoice':shedular_date,'back_date':True})
                wf_service.trg_validate(uid, 'account.invoice', invoices_ids[0], 'invoice_open', cr) 
        ...
        for merge_inv in invoices:
            if invoices_ids and len(invoices_ids)>1: 
                ...
                for merged_inv in merged_ids:
                    print 'before merged',merged_ids
                    self.write(cr,uid,[merged_inv],{'date_invoice':shedular_date,'back_date':True})
                    wf_service.trg_validate(uid, 'account.invoice', merged_inv, 'invoice_open', cr)
                    print 'after merged',merged_inv
...
#             context.update({'billing_ids':[{'partner_id':partner_id.id}]})
            ...
            for i in billing_obj.browse(cr,uid,billing_list,context):
                ...
                if end_date < shedular_date:
                    try:                                
                                    
                                 
                        billing.update({'st_date':st_date,
                                        'end_date':shedular_date,
                                        'partner_id':partner_id.id,
            ...
            if not billing_list:
                    year_to=self.get_month(cr,uid,[],shedular_date,{'yesr':1})
                    cr.execute("select date_start from account_fiscalyear where date_start <= '" + str(shedular_date) + "' and date_stop >='" + str(shedular_date) + "'")
                    date_start = cr.fetchone()
                    date_start = date_start and date_start[0] or ''                        
                    billing.update({'st_date': date_start,
                                    'end_date':shedular_date,
...
#                'price_unit'      : fields.float('Unit Price', digits_compute= dp.get_precision('Product Price'), readonly=True, states={'draft': [('readonly', False)]} ),
    ...
    def _default_account_id(self, cr, uid, context=None):
        ...
        if context.get('type') in ('in_refund','in_invoice'):
            cr.execute("select id from account_account where lower(name) like 'purchase of wood%'")
