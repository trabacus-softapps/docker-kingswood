    ...
    def write(self, cr, uid, ids,vals, context=None):
        ...
        for case in self.browse(cr,uid,ids):
            if 'back_date' in vals:
                ...
                if stock_in:
                    cr.execute("""select invoice_id from incoming_shipment_invoice_rel where in_shipment_id in %s""",(tuple(stock_in),))
                    invoice_in = [x for x in cr.fetchall()]

                if stock_out:
                    cr.execute("""select invoice_id from supp_delivery_invoice_rel where del_ord_id in %s""",(tuple(stock_out),))
                    invoice_in = [x for x in cr.fetchall()]                    
                
                if invoice_in:
                    print 'invoice_in',invoice_in
                    cr.execute("""update account_invoice set back_date = """+str(vals['back_date'])+""" where id in %s""",(tuple(stock_out),))
...
#         return super(account_invoice,self).unlink(cr, uid, ids,context)
    ...
    def create_facilitator_inv(self, cr, uid, ids, context=None):
        ...
        today = context.get('shedular_date','2015-10-08')
        ...
        # to be removed after the july 10 2015 by mani
        dummy_ids.append(2051) # Syed Ahmedullah-T
        dummy_ids.append(2052) # Syed Junath-T
        dummy_ids.append(2053) # M.Ramakrishna-T
        dummy_ids.append(2054) # S.Ramakrishna-T
        dummy_ids.append(1572) # Kingswood Suppliers Pvt. Ltd.(TN)
         # to be removed after the oct 10 2015 by mani
...
#         if state:
        cr.execute("""select distinct sp.id from stock_picking sp 
                        ...
                        where sp.date::date >= '2015-07-01' and sp.date::date <= '2015-10-08' and type = 'out' 
...
#         date::date >= %s::date and state in ('done','freight_paid') and sup_invoice=False""",(str(shedular_date),)) 
        ...
        if stock_ids_out:
            ...
            #self.write(cr,uid,stock_ids_out,{'user_id':1})  
...
#         stock_ids_out = stock_obj.search(cr,uid,[('id','in',stock_ids_out),('paying_agent_id','not in',dummy_ids)])
        ...
        cr.execute("""select distinct sp.id from stock_picking sp 
                        left outer join res_country_state rs on rs.id = sp.state_id
                        where sp.date::date >= '2015-07-01' and sp.date::date <= '2015-10-08' and type = 'in' 
                        and sp.date::date <= '"""+str(shedular_date)+"""'::date and sup_invoice = False                        
        ...
        if stock_ids_in:
            ...
            #self.write(cr,uid,stock_ids_in,{'user_id':1})
        if stock_ids_out or stock_ids_in:
            if stock_ids_out:
                cr.execute("""select sp.id from stock_picking sp where sp.date::date >= '2015-07-01'::date and sp.date::date <= '2015-10-08' and sp.id not in (SELECT dr.del_ord_id FROM supp_delivery_invoice_rel dr inner 
