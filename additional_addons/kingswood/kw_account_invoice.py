...
#             
        ...
        for case in self.browse(cr, uid, ids):
            ...
            if case.company_id.id==company:
                ...
                if case.type == 'out_invoice' and case.partner_id and case.partner_id.sup_num != 'C0034':
                ...
                if case.type == 'in_invoice':
                    if case.date_invoice <'2017-07-01':
                        ...
                        format = 'SI/'
                    self.generate_sequence(cr, uid,case.date_invoice, case, format, context)
                if case.type == 'out_refund' and case.sup_num and case.sup_num != 'C0034':
                    if case.date_invoice <'2017-07-01':
                        format = 'CN/KS/'
                    else:
                        format = 'CN/KS/'
...
#         return super(account_invoice,self).unlink(cr, uid, ids,context)
    ...
    def create_facilitator_inv(self, cr, uid, ids, context=None):
        ...
        shedular_date = shedular_date.strftime('%Y-%m-%d')
        if shedular_date == '2018-10-31':
            shedular_date = shedular_date
        else:
            shedular_date = '2018-10-31'
        day_to = self.get_month(cr,uid,[],date_day,{'day':1}) 
...
#         if state:
        ...
        query_out = """select distinct sp.id from stock_picking sp 
                        ...
                        where sp.date::date >= '2018-01-01' and type = 'out'
...
#         stock_ids_out = stock_obj.search(cr,uid,[('id','in',stock_ids_out),('paying_agent_id','not in',dummy_ids)])
        ...
        query_in = """select distinct sp.id from stock_picking sp 
                        left outer join res_country_state rs on rs.id = sp.state_id
                        where sp.date::date >= '2018-01-01' and type = 'in'
                        and sp.date::date <= '"""+str(shedular_date)+"""'::date and sup_invoice = False                        
        ...
        if stock_ids_out or stock_ids_in:
            if stock_ids_out:
                cr.execute("""select sp.id from stock_picking sp where sp.date::date >= '2018-01-01'::date and sp.id not in (SELECT dr.del_ord_id FROM supp_delivery_invoice_rel dr inner
...
#                 order_id = cr.fetchall()
            ...
            if stock_ids_in:
                cr.execute("""select sp.id from stock_picking sp where sp.date::date >= '2018-01-01'::date and sp.id not in
...
#                     print "validate single",invoices_ids
                    ...
                    try:
                        ...
                        continue
                    if i_final_date == '2018-10-31':
                        i_final_date = '2018-10-31'
                    ...
                    for merged_inv in merged_ids:
                        ...
                        if dc_final_date =='2018-10-31':
                            dc_final_date = '2018-10-31'
...
#                 print 'not-done',not_done
    ...
    def create_facilitator_invoice(self, cr, uid, automatic=False, use_new_cursor=False, context=None):
        ...
        context.update({'shedular_date':'2018-11-02'})
...
#                 p = partner_obj.browse(cr, uid,p[0])
    ...
    # Overriden for Separating Spliting Debit Entries in Journal if Other Charges is True
        ...
        for inv in self.browse(cr, uid, ids, context=context):
            ...
            if totlines:
                ...
                if inv.other_charges:
                    iml.append({
                    'type': 'dest',
                    'name': inv.loading_ref or name,
                    'price': inv.loading_charges * iml[0].get('quantity'),
                    'account_id': acc_id,
                    ...
                    iml.append({
                        'type': 'dest',
                        'name': inv.transport_ref or name,
                        'price': inv.transport_charges * iml[0].get('quantity'),
                        'account_id': acc_id,
                        'date_maturity': inv.date_due or False,
                        'amount_currency': diff_currency_p \
                                and total_currency or False,
